...
import {SIGN_IN,SIGN_OUT, REGISTER,FETCH_END,FETCH_START,UPDATE_BALANCE} from './constants'
...
export const updateBalance=()=>{
    return async (dispatch)=>{
        try{
            dispatch(fetchStart());
            const {data:resp}= await api.get('pay/balance',{headers:{Authorization:`Bearer ${localStorage.getItem("jwt")}`}});
